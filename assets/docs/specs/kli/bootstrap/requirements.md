...
- **`withCli`**: The single entry point. Accepts metadata, globals, deps,
  middleware, and commands. Returns a runnable CLI instance.
- **`withCommand`**: Declares a command — its args, opts, description,
  optional middleware, and handler.
- **`run`**: Executes the CLI instance. Parses argv, validates, runs
  middleware chain, calls the matched command handler.
...
- **`ctx`**: The unified context object every handler and middleware receives.
  Contains `args`, `opts`, `deps`, and `raw`.
...
4. THE `kli` package SHALL export testing utilities from a separate entry
   point: `kli/testing` — never imported in production code
5. THE `kli` package runtime dependencies SHALL be limited to `neverthrow` and
   `radash`; `@opentui/core` and `solid-js` SHALL be peer dependencies
...
1. `withCli(options)` SHALL accept:
   - `globals`: global opts available to every command
   - `deps`: the consumer's typed dependency object — built externally,
     passed in as-is
   - `middleware`: array of global middleware functions
   - `commands`: record of command name → command definition
2. `withCli` SHALL read `name`, `version`, and `description` from the
   consumer's `package.json` automatically — these SHALL NOT be manually
   specified
3. `withCli` SHALL return a typed CLI instance accepted by `run()`
...
5. WHEN `commands` is an empty record, `withCli` SHALL still return a valid
   CLI instance that prints help on `run()`
...
4. `opts` in `ctx` SHALL be the merge of global opts and command-local opts —
   local opts take precedence on conflict
5. `deps` in `ctx` SHALL be the exact typed object passed to `withCli` —
   no wrapping, no transformation
6. `raw` in `ctx` SHALL be the original `process.argv` slice after the
   command name
...
2. THE supported argument definition shape SHALL be:
   ```typescript
   { type: 'string' | 'number' | 'boolean', required?: boolean, desc?: string }
   ```
...
9. WHEN the same option appears multiple times in argv (e.g.
   `--tag a --tag b`), the values SHALL be collected into `string[]`
...
1. `run(cli)` SHALL execute in this order:
...
2. `run()` SHALL be the only function that calls `process.exit()`
3. ALL errors from steps 1–10 SHALL be caught, printed to stderr with
   context, and result in exit 1
4. `run()` SHALL be async and SHALL be awaited in `index.ts`
...
3. `withTui(App)` SHALL register the Solid root component with the CLI
   instance
4. THE Solid root component SHALL receive `{ deps, opts }` as props — `opts`
   contains the resolved global opts
...
7. WHEN no TUI is registered via `withTui`, TTY mode with no command SHALL
   fall back to `printHelp()` + exit 0
